...
func alertLink(alert newshound.NewsAlertLite) string {
	return fmt.Sprintf("http://newshound1.newsai.org/#/calendar?start=%s&display=alerts&alert=%s",
		alert.Timestamp.Format("2006-01-02"),
		alert.ID.Hex())
}

func eventLink(event newshound.NewsEvent) string {
	return fmt.Sprintf("http://newshound1.newsai.org/#/calendar?start=%s&display=events&event=%s",
